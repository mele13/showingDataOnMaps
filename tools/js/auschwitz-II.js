...
function init() {
    ...
    loader.load("../osm/auschwitz-II.osm", function (text) {
        ...
        for (let i = 0; i < x.length; i++) {
            ...
            for (let j = 0; j < tags.length; j++) {
                if (tags[j].hasAttribute("k")) {
                    ...
                    if (tags[j].getAttribute("v") == "ruins") { interest = 3; break; }
                    if (tags[j].getAttribute("v") == "womenscamp") { interest = 4; break; }
                    if (tags[j].getAttribute("v") == "Block 31") { interest = 5; break; }
                    if (tags[j].getAttribute("v") == "BIIb_Terezin") { interest = 6; break; }
                    if (tags[j].getAttribute("v") == "camp_street_b") { interest = 7; break; }
                    // if (tags[j].getAttribute("v") == "camp_street_a") { interest = 8; break; }                    
                    if (tags[j].getAttribute("v") == "gas_chambers") { interest = 9; break; }                  
                }
            ...
            if (interest > 0) {
                ...
                switch (interest) {
                    ...
                    case 3: drawBuildings(points, 0x00FFF7); break; // Ruins
                    case 4: drawBuildings(points, 0x0B09AC); break; // Block 22b
                    case 5: drawBuildings(points, 0xF448FF); break; // Block 31
                    case 6: drawBuildings(points, 0xFBFF00); break; // Terezin Family Camp
                    case 7: drawBuildings(points, 0xFF7F48); break; // Camp Street B
                    case 8: drawBuildings(points, 0x0B09AC); break; // Camp Street A
                    case 9: drawBuildings(points, 0xC30000); break; // Gas chambers
                }
                
                drawSphere(0.8, 2.5, 0, 0.13, 10, 10, 0x7CFF00); // Auschwitz II Historical Gate
            }
        }
